...
export default function PlantDisease() {
  ...
  const knownClasses = [
    ...
    "Tomato___Bacterial_spot", "Tomato___Early_blight", "Tomato___Late_blight", "Tomato___healthy", "Tomato___Tomato_Yellow_Leaf_Curl_Virus",
    ...
    if (file && model) {
      ...
      try {
        ...
        img.onload = async () => {
          ...
          setTimeout(() => {
            if (!knownClasses.includes(bestPrediction.className) || bestPrediction.probability < 0.99) {
              setResult({
                unrecognized: true,
                message: 'The uploaded image is not recognized as a valid plant leaf. Please try again with a clear leaf image.',
              });
            } else {
              setResult({
  ...
  const getTreatmentRecommendations = (disease) => {
    const treatments = {
      ...
      "Apple___Cedar_apple_rust": ["Remove nearby junipers", "Apply myclobutanil in early spring"],
      "Apple___Black_rot": ["Remove infected limbs", "Apply thiophanate-methyl or captan"],
      "Apple___Apple_scab": ["Prune affected leaves", "Apply captan fungicide"],

      "Potato___Early_blight": ["Use certified seed", "Apply chlorothalonil or mancozeb fungicides"],
      "Potato___Late_blight": ["Remove infected plants", "Apply fungicides with fluazinam or cyazofamid"],
      "Potato___healthy": ["No action needed"],

      "Tomato___Bacterial_spot": ["Use resistant seeds", "Apply copper-based sprays"],
      "Tomato___Early_blight": ["Rotate crops", "Use fungicides like chlorothalonil"],
      "Tomato___Late_blight": ["Remove affected plants", "Use resistant varieties", "Apply protective fungicides"],
      "Tomato___healthy": ["No action needed"],
      "Tomato___Tomato_Yellow_Leaf_Curl_Virus": ["Remove infected plants", "Control whiteflies", "Use virus-resistant seeds"],

      "Peach___healthy": ["No action needed"],
      "Peach___Bacterial_spot": ["Apply copper sprays", "Remove infected twigs and fruit"],

      "Grape___Black_rot": ["Prune infected shoots", "Apply myclobutanil early season"],
      "Grape___Esca_(Black_Measles)": ["Avoid pruning wounds", "Remove infected vines"],
      "Grape___healthy": ["No action needed"],
      "Grape___Leaf_blight_(Isariopsis_Leaf_Spot)": ["Improve airflow", "Apply copper-based fungicides"],

      "Cherry_(including_sour)___healthy": ["No action needed"],
      "Cherry_(including_sour)___Powdery_mildew": ["Prune to increase airflow", "Apply sulfur fungicides"],

      "Corn_(maize)___Cercospora_leaf_spot Gray_leaf_spot": ["Use resistant hybrids", "Apply strobilurin fungicides"],
      "Corn_(maize)___Common_rust_": ["Plant resistant varieties", "Use azoxystrobin fungicides"],
      "Corn_(maize)___healthy": ["No action needed"],
      "Corn_(maize)___Northern_Leaf_Blight": ["Use propiconazole fungicide", "Practice crop rotation"]
    };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {result && result.unrecognized && (
        <div id="detected" className="bg-yellow-100 text-yellow-800 p-4 rounded-lg shadow-md">
